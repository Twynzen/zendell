...
def build_system_context(db: MongoDBManager, user_id: str, stage: str) -> str:
    ...
    context = (
        ...
        "En 'clarifier_last_hour', profundiza en detalles (qué, cuándo, dónde, cómo, con quién, por qué) de las actividades pasadas. "
    ...
    print(f"[DEBUG] System Context: {context}")
    return context
...
def ask_gpt_in_context(db: MongoDBManager, user_id: str, user_prompt: str, stage: str) -> str:
    ...
    print(f"[DEBUG] ask_gpt_in_context - Prompt final: {chat}")
    response = ask_gpt_chat(chat, model="gpt-3.5-turbo", temperature=0.7)
    print(f"[DEBUG] ask_gpt_in_context - Respuesta del LLM: {response}")
...
def orchestrator_flow(user_id: str, last_message: str, global_state_override: dict = None) -> Dict[str, Any]:
    ...
    print(f"[DEBUG] Orchestrator - Estado inicial: {stage}")
    global_state = {
        "user_id": user_id,
    ...
    print(f"[DEBUG] Orchestrator - Estado tras override: {stage}")
    ...
    if stage == "initial":
        ...
        # Forzamos la transición a clarifier_last_hour
        stage = "clarifier_last_hour"
        state["conversation_stage"] = stage
        db.save_state(user_id, state)
        print(f"[DEBUG] Orchestrator - Transición a clarifier_last_hour forzada.")
        ...
        print(f"[DEBUG] Orchestrator - Preguntas generadas (clarifier_last_hour): {questions}")
        if questions:
        ...
        global_state = clarifier_node(global_state)
        questions = global_state.get("clarification_questions", [])
        print(f"[DEBUG] Orchestrator - Preguntas generadas (clarifier_next_hour): {questions}")
        if questions:
    ...
    print(f"[DEBUG] Orchestrator - Estado final: {stage}")
