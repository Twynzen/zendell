...
class ZendellConfigUI:
    ...
    def start_system(self):
        ...
        try:
            # Find the main.py script relative to this script's location
            current_dir = os.path.dirname(os.path.abspath(__file__))
            main_script = os.path.join(current_dir, "zendell", "main.py")
            
            # Check if the main script exists
            if not os.path.exists(main_script):
                self.status_var.set(f"Error: Could not find {main_script}")
                messagebox.showerror("Error", f"Could not find the main script at:\n{main_script}")
                return
